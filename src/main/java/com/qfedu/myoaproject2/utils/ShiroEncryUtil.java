...
public class ShiroEncryUtil {
    ...
    public static String aesEncode(Key key, String data) {
        return new AesCipherService().encrypt(
                data.getBytes(), key.getEncoded()).toBase64();
    }

    //AES 解密
    public static String aesDecode(Key key, String data) {
        return new String(new AesCipherService().decrypt(
                Base64.decode(data), key.getEncoded()).getBytes());
    }
